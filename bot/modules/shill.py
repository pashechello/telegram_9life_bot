...
EDIT_SHILL_POST_HANDLER=ConversationHandler(
    entry_points=[MessageHandler(Filters.regex('^📝 Изображение$'),edit_shill)],
    states={
        POST:[
        ...
        SHILL1:[MessageHandler(Filters.text & ~Filters.regex('^🚫 Завершить$'),add_post_1)],
        SHILL2:[MessageHandler(Filters.text & ~Filters.regex('^🚫 Завершить$'),add_post_2)],
        SHILL3:[MessageHandler(Filters.text & ~Filters.regex('^🚫 Завершить$'),add_post_3)]
        
    },
    fallbacks=[
        MessageHandler(Filters.regex('^🚫 Завершить$'),cancel_shill),
        MessageHandler(Filters.regex('^🔙 Назад$'),cancel_shill),
        ]
...
SET_WELCOME_TEXT_HANDLER=ConversationHandler(
    entry_points=[MessageHandler(Filters.regex('^✉️ Текст$'),set_welcome_message)],
    states={
        WELCOME : [MessageHandler(Filters.text & ~Filters.regex('^🚫 Завершить$'),process_welcome_text)]
    },
    fallbacks=[
        MessageHandler(Filters.regex('^🚫 Завершить$'),cancel_shill)]
)
...
SET_WELCOME_IMAGE_HANDLER=ConversationHandler(
    entry_points=[MessageHandler(Filters.regex('^🖼 Изображение$'),set_welcome_image)],
    states={
